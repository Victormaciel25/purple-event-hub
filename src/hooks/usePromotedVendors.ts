...
export const usePromotedVendors = () => {
  ...
  const fetchVendorsWithPromotion = async () => {
    try {
      ...
      // Para fornecedores promovidos, filtrar apenas os que estão dentro de 100km (se temos localização)
      let nearbyPromotedVendors = promotedVendors;
      if (location) {
        nearbyPromotedVendors = promotedVendors.filter(vendor => 
      ...
      const finalVendors = [...nearbyPromotedVendors, ...normalVendors];
